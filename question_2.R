...
pacman::p_load(tidyverse, spacyr, tidymodels, 
               textrecipes, tidytext, finetune)
# note: all models and results already saved in RData file
load("~/bball.RData")
cf_perf = cf_perf %>%
...
# data for paired t-test (n = 74)
cf_data2 = cf_text %>% 
  arrange(cf_pick) %>%
  distinct(idPlayer, .keep_all = T) %>%
  arrange(pick) %>% distinct(cf_ProtectionId, .keep_all = TRUE) %>%
  filter(!is.na(vorp) & !is.na(cf_vorp))
# table results for paired t-test
cf_sum = cbind(cf_data2 %>% summarize(pick = round(mean(pick),2), 
               ...
               cf_data2 %>% summarize(pick = round(mean(cf_pick),2), 
  ...
  mutate(Difference = round(Substitute - Protected,2),
         `T-Value` = NA,
         `P-Value` = NA)
cf_sum$`T-Value`[1] = round(t.test(cf_data2$pick, cf_data2$cf_pick, paired = TRUE)$statistic,2)
cf_sum$`T-Value`[2] = round(t.test(cf_data2$games, cf_data2$cf_games, paired = TRUE)$statistic,2)
cf_sum$`T-Value`[3] = round(t.test(cf_data2$PER, cf_data2$cf_per, paired = TRUE)$statistic,2)
cf_sum$`T-Value`[4] = round(t.test(cf_data2$bpm, cf_data2$cf_bpm, paired = TRUE)$statistic,2)
cf_sum$`T-Value`[5] = round(t.test(cf_data2$ws_48, cf_data2$cf_ws48, paired = TRUE)$statistic,2)
cf_sum$`T-Value`[6] = round(t.test(cf_data2$vorp, cf_data2$cf_vorp, paired = TRUE)$statistic,2)
cf_sum$`P-Value`[1] = round(t.test(cf_data2$pick, cf_data2$cf_pick, paired = TRUE)$p.value,3)
cf_sum$`P-Value`[2] = round(t.test(cf_data2$games, cf_data2$cf_games, paired = TRUE)$p.value,3)
cf_sum$`P-Value`[3] = round(t.test(cf_data2$PER, cf_data2$cf_per, paired = TRUE)$p.value,3)
cf_sum$`P-Value`[4] = round(t.test(cf_data2$bpm, cf_data2$cf_bpm, paired = TRUE)$p.value,3)
cf_sum$`P-Value`[5] = round(t.test(cf_data2$ws_48, cf_data2$cf_ws48, paired = TRUE)$p.value,3)
cf_sum$`P-Value`[6] = round(t.test(cf_data2$vorp, cf_data2$cf_vorp, paired = TRUE)$p.value,3)

## svr data n = 110 with non-missing VORP
set.seed(123)
